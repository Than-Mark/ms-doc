...
Therefore, the order parameter $\Delta \Omega$ can be used to distinguish the Ring state from the Disorder state.
...
\begin{table}
    ...
    \begin{ruledtabular}
        ...
        \end{tabular}
    \end{ruledtabular}
    \footnotetext[1]{Note that the $R_s$ of Double-Clusters state is closer to $1$ than Multi-Clusters.}
\end{table}
...
We first consider the transition between the Disorder and Ring states. The oscillators in the Ring state move in a circular trajectory with a constant rotational radius $r_i=v/\omega_i$. The oscillators in the Disorder state move in a circular trajectory with a constant rotational radius $r_i=v/\omega_s$, where $\omega_s$ is the average of the natural frequencies of the oscillators. The critical line $\lambda_{c1}$ is determined by the condition that the rotational radius of the oscillators in the Ring state is equal to the rotational radius of the oscillators in the Disorder state. We have:
