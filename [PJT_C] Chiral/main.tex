...
The real-time rotational radius is $r_i(t)=\left| \mathbf{c}_i(t)-\mathbf{r}_i(t) \right|$. We found that the real-time rotational radius is almost constant and close to $v/\omega_i$ for each oscillator in the Disorder state, as shown in Fig.~\ref{fig:radiusOmega}a. The estimation results of four states's real-time rotational centers are shown in Appendix \ref{sec:estimationCenter}.
...
The reason of using the distance between centers instead of the distance between oscillators' positions is that the oscillators in the Ring state are uniformly distributed on the circular trajectory, and the distance between them is much larger than the distance between the centers.
...
\vspace{-0.7cm}
\begin{table}[H]
    \caption{\label{tab:orderParam} Key properties of the states. 
    }
    \begin{ruledtabular}
        \begin{tabular}{lccc}
        ...
        Swarm (Multi-Clusters)&$> 0$&$\approx 1$&$= 0$\\
    ...
    Note that the $R_s$ of Double-Clusters state is closer to $1$ than Multi-Clusters.
\end{table}
...
\section{\label{sec:estimationCenter} ESTIMATION RESULTS OF REAL-TIME ROTATIONAL CENTERS}

The real-time rotational centers are estimated with method in Fig.~\ref{fig:CenterEps} and Eq.~(\ref{eq:linearEquations}). The estimation results of four states are shown in Fig.~\ref{fig:etimateCenter}.

\begin{figure}[H]
    \includegraphics[width=0.48\textwidth]{./figs/etimateCenter.png}
    \caption{
