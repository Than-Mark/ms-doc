...
Under the assumption of no coupling, the center $\mathbf{c}_i$ of the $i$th oscillator moves on a circular trajectory with radius $r_i=v/\omega_i$ and angular velocity $\omega_i$ according to Eq.~(\ref{eq:circlemotionx}).
...
\end{equation}
Due to the symmetry of the ring, we can consider the interaction between only two oscillators. For the $i$th and $j$th oscillators, the distance between their rotation centers $\left| \mathbf{c}_j-\mathbf{c}_i \right|$ is
...
\end{equation}
Furthermore, due to the symmetry of coupling from Eq.~(\ref{eq:dotthetai}), The phase of the two oscillators after being coupled will have a pair of symmetrical angular offsets compared to those without coupling:
\begin{equation}
    \begin{array}{c}
