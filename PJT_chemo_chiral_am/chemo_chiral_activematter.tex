...
\begin{equation}
    \frac{\partial}{\partial t}\rho ^{1,2}\left( \mathbf{r},\theta ,t \right) =-\frac{\partial}{\partial \theta}\left( \rho ^{1,2}v_{\theta}^{1,2} \right) -\nabla \cdot \left( \rho ^{1,2}\mathbf{v}_{\mathbf{r}}^{1,2} \right) \;, \label{eq:continuumModel}
\end{equation}
where $\rho^{1,2}\left( \mathbf{r},\theta ,t \right)$ is the probability density of swarmalators of species 1 or 2 at position $\mathbf{r}$ and orientation $\theta$ at time $t$, and $\mathbf{v}_{\mathbf{r}}^{1,2}$ and $v_{\theta}^{1,2}$ are the velocity fields in the position and orientation space, respectively. The velocity fields are given by
\begin{subequations}
    \begin{align}
        ...
        &\mathbf{v}_{\mathbf{r}}^{1,2}\left( \mathbf{r},\theta ,t \right) =v\mathbf{p}\left( \theta \right) -\int d\theta 'd\mathbf{r}'\rho ^{1,2}\left( \mathbf{r}',\theta ',t \right)\mathbf{I}\left( \mathbf{r}-\mathbf{r}' \right) \;,
    \end{align}
...
\begin{equation}
    \begin{aligned}
        \frac{\partial}{\partial t}\rho ^{1,2}\left( \mathbf{r},\theta ,t \right) =&-\omega \partial _{\theta}\rho ^{1,2}-\left| \nabla c_{1,2} \right|\partial _{\theta}\left[ \rho ^{1,2}\sin \left( \varphi _{c_{1,2}}-\theta \right) \right]\\
        &-v\mathbf{p}\left( \theta \right) \cdot \nabla \rho ^{1,2}-\nabla \cdot \int{d\theta \prime d\mathbf{r}\prime \rho ^{1,2}\left( \mathbf{r}\prime ,\theta \prime ,t \right) \mathbf{I}\left( \mathbf{r}-\mathbf{r}\prime \right)}\;.
    \end{aligned}
\end{equation}
