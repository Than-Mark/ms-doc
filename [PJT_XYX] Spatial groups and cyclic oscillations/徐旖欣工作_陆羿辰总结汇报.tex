...
\begin{cases}
	...
	\dot{\theta}_1=\omega _1+\lambda f_H\left( d_0-d \right) \sin \left( \theta _2-\theta _1 \right)\\
	\dot{x}_2=v\cos \theta _2\\
	\dot{y}_2=v\sin \theta _2\\
	\dot{\theta}_2=\omega _2+\lambda f_H\left( d_0-d \right) \sin \left( \theta _1-\theta _2 \right)\\
\end{cases}
$$
其中$f_H\left( x \right)$为Heaviside阶跃函数,即
$$
f_H\left( x \right)=\begin{cases}
	1,&		x\geqslant 0\\
	0,&		x<0\\
\end{cases}
$$
$d$为两振子之间的距离,即$d=\sqrt{\left( x_1-x_2 \right) ^2+\left( y_1-y_2 \right) ^2}$. 考虑到Heaviside函数的不连续性, 以近似函数$f_{F}\left( x \right)$代替$f_H\left( x \right)$,即
$$
f_F\left( x \right) =\frac{1}{e^{\left( d-d_0 \right) /\mu}+1}
$$
其中, $\mu$为参数, 满足$\mu>0$, 且当$\mu\rightarrow0^{+}$时, 有$f_{F}\left( x \right)\rightarrow f_H\left( x \right)$. 已知振子
...
其中$(N-1)$是集群中的振子数, $S_i$是$\mathbf{x}_i$为圆心,以$d_0$为半径的圆与集群重叠部分的面积, $S_s$是集群圆的面积. 由初等几何知识可得
...
$$
\begin{cases}
	\lambda _{c2}=\frac{\pi r_{s}^{2}\left( \omega _{\max}-\omega _{\min} \right)}{2\left( N-1 \right) \left( d_{0}^{2}\frac{\alpha}{2}+r_{s}^{2}\frac{\beta}{2}-r_sd_0\sin \frac{\alpha}{2} \right)}\\
	\beta =2\mathrm{arc}\cos \left( 1-\frac{d_{0}^{2}}{2r_{s}^{2}} \right)\\
	\alpha =\frac{2\pi -\beta}{2}\\
\end{cases}
$$
...
上式可以理解为振子$i$作用半径内的振子数等于单位面积下的振子数乘以其作用范围的面积. 由\ref{eq:eq3}式可知在给定的耦合强度下, $\left| \Delta \omega \right|$越大的振子越难以被同步到集群. $\Delta \omega =\omega _{\max}-\omega _{\min}$时取得最大值, 此时同手性的振子$i$最难被同步到集群. 从而临界耦合强度$\lambda_{c2}(d_0)$为
...
\begin{equation}\label{eq:eq7}
	\lambda _{c4}=\frac{L^2\left[ \omega _{\max}-\left( -\omega _{\max} \right) \right]}{N\pi d_{0}^{2}/2}=\frac{4L^2\omega _{\max}}{N\pi d_{0}^{2}}
\end{equation}
