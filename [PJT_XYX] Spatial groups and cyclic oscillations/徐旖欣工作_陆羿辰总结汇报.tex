...
\noindent\textbf{有限距离相位同步程度(截面序参量)}
...
\newpage
\noindent\textbf{聚类平均距离方差(截面时序序参量)}
$$
...
\newpage
\noindent\textbf{有限距离相位同步程度(各粒子/时序序参量)}
...
\begin{figure}[H]
	...
	\caption{有限距离相位同步程度(各粒子)}
	\label{fig:fig234t.2}
\end{figure}

\newpage
\noindent\textbf{有限距离相位同步程度(算数平均/时序序参量)}
