...
除了对全局平均频率差的讨论，还可以对各个集群的平均频率差进行讨论. 这里对集群的定义采用方法\ref{clustering}($d_{th}=0.3$)进行聚类，然后计算每一类(粒子数不足5个的分类剔除)中粒子的平均频率差，最后取所有类的平均频率差的算数平均.
...
其中，$N_{class}$为集群数，$N_k$为第$k$个集群的粒子数. $\overline{\,\,\cdot \,\,}$为集群内平均, 即$\bar{x}=\frac{1}{N_k}\sum_{i\in C_k}{x_i}$
