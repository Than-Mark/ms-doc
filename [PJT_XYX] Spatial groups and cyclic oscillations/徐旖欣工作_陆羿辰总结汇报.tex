...
考虑到同手性振子在形成环态时，存在旋转中心的聚集现象，因此可以假设在耦合强度与作用半径小于且充分接近临界值的情况下，同手性振子会聚集成环，如下图\ref{fig:fig23.1}(a)所示. 而在耦合强度与作用半径大于临界值且恰巧形成集群的情况下，同手性振子会聚集成集群，如下图\ref{fig:fig23.1}(b)所示.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./figs/circleSwarmEdges1.png}
	\vspace{-1cm}
	\caption{\textbf{集群态与环态临界状态下的空间分布}. (a)环态. 最外侧的粒子在; (b)集群态.}
	\label{fig:fig23.1}
\end{figure}
