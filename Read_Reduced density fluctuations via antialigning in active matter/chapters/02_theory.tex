...
\begin{remark}[各项物理意义逐项解读]
\textbf{密度方程（\ref{eq:langevin-rho}）：}
\begin{itemize}
    \item $\partial_t \tilde{\rho}$：总密度场随时间的局域变化率。
    \item $-v\nabla\tilde{m}$：\textbf{磁化强度梯度驱动的密度流}。由于 $(+)$ 粒子向右、$(-)$ 粒子向左运动，若某处右侧的向右粒子多于左侧（即 $\nabla\tilde{m}>0$），则该处密度会下降，反之上升。负号体现了粒子从磁化强度高的区域向低的区域净输运。
\end{itemize}

\textbf{磁化强度方程（\ref{eq:langevin-m}）：}
\begin{itemize}
    \item $\partial_t \tilde{m}$：磁化强度（即向右与向左粒子数之差）随时间的变化率。
    \item $-v\nabla\tilde{\rho}$：\textbf{密度梯度驱动的磁化强度流}。若密度向右递增（$\nabla\tilde{\rho}>0$），则更多的向右运动粒子会从左侧流入，导致局域磁化强度下降。
    \item $-r_0\tilde{m}$：\textbf{自发转换导致的磁化弛豫}。$r_0$ 是自发翻转率，它使 $(+)$ 与 $(-)$ 粒子相互转换，驱使系统趋向非极化态（$\tilde{m}=0$），是一个指数衰减项。
    \item $-r_1\tilde{\rho}\tilde{m}$：\textbf{反排列相互作用导致的非线性弛豫}。这是本文最核心的项：局部磁化越强，粒子因``反排列''机制翻转到相反方向的速率越快；且该效应与局域总密度成正比（密度越高，碰撞/相互作用越频繁）。该项是抑制磁化、进而抑制密度涨落的关键。
    \item $i\sqrt{2r_1\tilde{\rho}}\,\xi$：\textbf{虚噪声项}。这是 Poisson 表示法的标志性结果。转换过程的离散性和非线性导致磁化强度动力学存在内禀随机涨落；扩散矩阵 $D$ 的负定性使得噪声系数为纯虚数，意味着系统呈现 \textbf{sub-Poissonian} 统计（涨落窄于 Poisson 分布）。注意密度方程中完全没有噪声，这是该模型涨落被强烈抑制的重要信号。
\end{itemize}
\end{remark}
...
\begin{derivation}
将二阶方程~(\ref{eq:second-order-eq})的\textbf{确定性}部分（先忽略噪声）写到 Fourier 空间，设 $\nabla^2 \to -k^2$：
...
\begin{notebox}[符号说明：$\delta\tilde{\rho}_k$ 的含义]
回顾式~(\ref{eq:linearization})，我们将总密度写为均匀背景与小扰动之和：$\tilde{\rho} = \rho_0 + \delta\tilde{\rho}$。下标 $k$ 表示对空间坐标做 Fourier 变换后的\textbf{波数分量}：
\[
\delta\tilde{\rho}_k(t) = \int \dd x\, e^{-ikx}\,\delta\tilde{\rho}(x,t)
\quad\text{（连续）}\qquad\text{或}\qquad
\delta\tilde{\rho}_k(t) = \sum_j e^{-ikj}\,\delta\tilde{\rho}_j(t)
\quad\text{（离散晶格）}
\]
因此 $\delta\tilde{\rho}_k$ 就是密度涨落在波数 $k$ 处的\textbf{Fourier 模式振幅}。对它求时间的导数时，空间导数 $\nabla$ 被替换为 $ik$（或 $\nabla^2 \to -k^2$），从而将偏微分方程化为关于每个 $k$ 的常微分方程。
\end{notebox}
这正是\textbf{阻尼谐振子}的标准形式，其中：
\begin{itemize}
    \item $\gamma = r_0 + r_1\rho_0$ \quad（阻尼系数，来自自发转换与反排列相互作用）
    \item $\omega_0^2 = v^2 k^2$ \quad（固有频率的平方，来自粒子的定向流动）
\end{itemize}

阻尼谐振子的特征方程为 $\lambda^2 + \gamma\lambda + \omega_0^2 = 0$，解得两个特征值：
\begin{equation}\label{eq:eigenvalues-full}
\lambda_{\pm} = \frac{-\gamma \pm \sqrt{\gamma^2 - 4\omega_0^2}}{2}
\end{equation}

\textbf{过阻尼条件}：当 $\gamma^2 \gg 4\omega_0^2$ 时，根号内为正，两个特征值都是\textbf{实数且为负}，系统不发生振荡而直接衰减。在本模型中：
\[
\gamma^2 = (r_0 + r_1\rho_0)^2, \qquad 4\omega_0^2 = 4v^2 k^2
\]
由于 $\gamma$ 是常数而 $\omega_0 \propto k$，在\textbf{大尺度极限 $k \to 0$}下，条件自动满足！这就是系统在大尺度上过阻尼的根本原因：粒子间的快速转换（$\gamma$）抹平了由流动引起的振荡趋势。

\begin{remark}[为什么要对根号展开？基于什么近似？]
如果不做展开，特征值~(\ref{eq:eigenvalues-full}) 被包在一个根号里，我们\textbf{无法直接读出}两个模式的标度行为。过阻尼极限的核心是 $\gamma^2 \gg 4\omega_0^2$，即
\[
\epsilon \equiv \frac{4\omega_0^2}{\gamma^2} = \frac{4v^2 k^2}{(r_0 + r_1\rho_0)^2} \ll 1
\]
这是一个\textbf{小参数}。将根号改写为
\[
\sqrt{\gamma^2 - 4\omega_0^2} = \gamma\sqrt{1 - \epsilon}
\]
并对 $\sqrt{1 - \epsilon}$ 做 Taylor 展开（到一阶）：
\[
\sqrt{1 - \epsilon} = 1 - \frac{\epsilon}{2} + O(\epsilon^2) \approx 1 - \frac{2\omega_0^2}{\gamma^2}
\]
这样根号就被\textbf{线性化}了，代入后两个特征值的 $k$ 依赖关系才变得透明：一个正比于 $-\gamma$（快模式），另一个正比于 $-k^2$（慢模式）。如果保留根号，后续对结构因子、扩散系数的标度分析都会受阻。
\end{remark}
...
\]
代入式~(\ref{eq:eigenvalues-full})，得到两个模式：
\begin{align}
...
\begin{itemize}
    \item $\lambda_-$ 是\textbf{快模式}（$\sim -\gamma$），在极短时间内 $e^{\lambda_- t}$ 就衰减到零。
    \item $\lambda_+$ 是\textbf{慢模式}（$\sim -k^2$），决定了系统的长时间行为。
\end{itemize}
...
\begin{remark}[过阻尼的另一种直观理解]
也可以直接从二阶方程~(\ref{eq:second-order-eq})看出过阻尼的本质：阻尼项 $\gamma\partial_t\delta\tilde{\rho}$ 的系数 $\gamma = r_0 + r_1\rho_0$ 在 $k \to 0$ 时远大于惯性项 $\partial_t^2\delta\tilde{\rho}$ 的等效系数（可取为 1）。因此可以\textbf{绝热地忽略惯性项} $\partial_t^2\delta\tilde{\rho} \approx 0$，立即得到：
\[
...
\]
这与式~(\ref{eq:eigenvalue})完全一致。物理上，这意味着磁化强度（快变量）被密度（慢变量）\textbf{奴隶}（slaved），系统没有振荡而是直接扩散弛豫。
\end{remark}
...
\begin{remark}[为什么 $N_S(k)$ 可以写成 $\rho_0 + \langle \delta\tilde{\rho}_k \delta\tilde{\rho}_{-k} \rangle$？]
这是 Poisson 表示法的一个标准恒等式。物理占据数 $n_i = n_i^+ + n_i^-$ 是速率为 $\alpha_i, \beta_i$ 的 Poisson 变量。对 Poisson 过程有：
\[
...
\end{align*}
第二项 $\delta_{ij}\rho_0$ 正是 Poisson 统计的\textbf{自关联}贡献（同一格点上的粒子数涨落服从 Poisson 分布，方差等于均值）。转到 Fourier 空间并对 $k \neq 0$ 的模式（此时 $\tilde{\rho}_k = \delta\tilde{\rho}_k$，因为均匀背景 $\rho_0$ 只贡献 $k=0$），即得到式~(\ref{eq:ns-definition})。
\end{remark}
